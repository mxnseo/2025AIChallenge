...
from collections import defaultdict    ## 추가코드
...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    if rank == 0:
        ...
        # 고침
        # tb_dict 평균 계산용
        tb_sums = defaultdict(float)   # key -> 합
        tb_counts = defaultdict(int)   # key -> 개수 (원하면 배치사이즈로 가중치 줄 수도 있음)
        ...
        if rank == 0:
            ...
            if tb_dict is not None:
                if batch_size is None:
                    ...
                    for k, v in tb_dict.items():
                        tb_sums[k] += float(v) * batch_size
                        tb_counts[k] += batch_size
            ####            
            disp_dict.update({
    ...
    if rank == 0:
        ...
        # 수정
        if tb_log is not None:
            tb_log.add_scalar('epoch_train/loss', losses_m.avg, cur_epoch)
            tb_log.add_scalar('epoch_train/learning_rate', cur_lr, cur_epoch)
            for k in tb_sums.keys():
                avg_v = tb_sums[k] / max(tb_counts[k], 1)
                tb_log.add_scalar(f'epoch_train/{k}', avg_v, cur_epoch)
