...
def add_random_noise(data_dict=None, config=None):
    """
    Adds random noise points to the point cloud, either in a rectangular or circular area.
    """
    ...
    num_noise_points = config.get('NUM_NOISE_POINTS', 500)
    noise_type = config.get('NOISE_TYPE', 'rectangular')

    num_original_features = points.shape[1]
    
    noise_points = None

    if noise_type == 'rectangular':
        noise_range = config.get('NOISE_RANGE', [-80, -80, -2, 80, 80, 4])
        noise_coords = np.random.uniform(
            low=[noise_range[0], noise_range[1], noise_range[2]],
            high=[noise_range[3], noise_range[4], noise_range[5]],
            size=(num_noise_points, 3)
        )
        noise_intensity = np.random.rand(num_noise_points, 1)
        noise_points = np.hstack([
            noise_coords, 
            noise_intensity, 
            np.zeros((num_noise_points, num_original_features - 4))
        ])

    elif noise_type == 'circular':
        radius = config.get('RADIUS', 60.0)
        z_range = config.get('Z_RANGE', [-2, 4])
        
        # 원 안에 점을 균일하게 생성하기 위해 사각형 내에서 생성 후 필터링
        # 필요한 점보다 1.5배 많이 생성하여 충분한 샘플 확보
        num_to_generate = int(num_noise_points * 1.5)
        
        angles = np.random.uniform(0, 2 * np.pi, num_to_generate)
        radii = np.sqrt(np.random.uniform(0, 1, num_to_generate)) * radius
        
        noise_x = radii * np.cos(angles)
        noise_y = radii * np.sin(angles)
        noise_z = np.random.uniform(z_range[0], z_range[1], num_to_generate)
        
        # 필요한 개수만큼만 선택
        noise_coords = np.vstack([noise_x, noise_y, noise_z]).T[:num_noise_points]
    ...
    if noise_points is not None:
        data_dict['points'] = np.vstack([points, noise_points.astype(points.dtype)])
    
    return data_dict

def random_downsample(data_dict=None, config=None):
    """
    Downsample based on probabilistic thresholds to handle ambiguous point counts.
    Keeps the z-binning strategy for physically meaningful sampling.
    """
    ...
    # 설정 값 불러오기
    upper_threshold = int(config.get('UPPER_THRESHOLD', 220000)) # 확실한 128ch 기준
    lower_threshold = int(config.get('LOWER_THRESHOLD', 150000)) # 확실한 64ch 기준
    probability = float(config.get('PROBABILITY', 0.5))        # 애매한 구간에 적용할 확률
    ...
    if n_points >= upper_threshold:
        # 포인트 수가 매우 많으면 100% 다운샘플링
        apply_downsample = True
    elif n_points > lower_threshold:
        # 애매한 구간에서는 설정된 확률에 따라 다운샘플링
        if random.random() < probability:
            apply_downsample = True
    
    # 다운샘플링을 적용하지 않기로 결정되면, 원본 그대로 반환
    if not apply_downsample:
        return data_dict
    # --- 로직 끝 ---

    # --- z-bin 다운샘플링 알고리즘 (기존과 동일) ---
