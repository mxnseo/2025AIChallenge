...
class DataAugmentor(object):
    ...
    # pcdet/datasets/augmentor/data_augmentor.py 파일의 DataAugmentor 클래스 내부에 추가

    def add_random_noise(self, data_dict=None, config=None):
        """ Wrapper for add_random_noise utility. """
        if data_dict is None:
            from functools import partial
            return partial(self.add_random_noise, config=config)
        return augmentor_utils.add_random_noise(data_dict=data_dict, config=config)
    #추가
    def random_downsample(self, data_dict=None, config=None):
        # 다른 augmentor들과 동일한 패턴: data_dict가 None이면 partial을 반환
        if data_dict is None:
            from functools import partial
            return partial(self.random_downsample, config=config)
        # 실제 동작은 augmentor_utils.random_downsample에 위임
        return augmentor_utils.random_downsample(data_dict=data_dict, config=config)
